...
const Career = () => {
  ...
  return (
    <div className="flex flex-col justify-center items-center relative">
      ...
      <div className="sticky top-20 bg-white rounded-3xl z-10 p-5 shadow-md w-full">
        <h2 className="text-5xl font-extrabold text-[#2c3a43]">Career Guidance</h2>

        {/* Dropdown to select category */}
        <div className='dropdown mt-5'>
          <label htmlFor="career_options" className="text-xl px-10">
            Choose the Category 
          </label>
          <select 
            id="career_options" 
            ...
            className="p-2 rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-orange-400"
          >
      ...
      <div className="subcategories-container w-full px-10 py-5">
        <h3 className=" text-lg sticky top-[220px] w-fit px-4 py-1 z-10 bg-white rounded-2xl">
          {selectedCategory ? (
          ...
          {(selectedCategory ? [careerdetails.find(category => category.category === selectedCategory)] : careerdetails).map((category, index) => (
            <div key={index} className='flex flex-wrap gap-10'>
              {category.sub_categories.map((subCategory, subIndex) => (
                <div
                  key={subIndex}
                  className="card flex flex-col w-[80%] hover:scale-[1.1] transition-all duration-300 justify-center items-center gap-5 mx-auto bg-gradient-to-br shadow-amber-100 shadow-2xl p-5 rounded-3xl drop-shadow-2xl from-[#fed0c5] to-[#8b47b5]"
                >
                  <h1 className="text-3xl font-bold text-orange-600">{subCategory.name}</h1>
